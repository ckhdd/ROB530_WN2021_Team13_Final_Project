...
class grid(object):
    def __init__(self, shape, size):
        if shape.ndim != 1 or size.ndim != 1:
            raise ValueError('Input arguments \"shape\" and \"size\" must have the same dimensionality.')
        if shape.shape != size.shape:
            raise ValueError('Input arguments \"shape\" and \"size\" must have the same shape.')
