...
class particlefilter:
    def __init__(self, count, start, posrange, angrange, 
        ...
        self.xukf = np.zeros((count,3,1))
        self.particles_pre = np.matmul(start, util.xyp2ht(dxyp))
        self.Pukf = 0.2*np.ones((count,3,3))
        self.Poutt = np.ones((count,3,3))
        self.particles_dyn = np.matmul(start, util.xyp2ht(dxyp))
    ...
    def measurement(self, X, poleparams):
        ...
        print( ind, self.polemeans.shape, self.polemeans[:, :2][0])
        Z = self.polemeans[:, :2][ind].reshape(-1,1) # (2*Nout,1)
        z_pred = polepos_w.T[:2].reshape(-1,1) # (2*Nout,1)
        return Z, z_pred

    def motion(self, Xin, mean, cov):
        T_r0_r1 = util.xyp2ht(np.random.multivariate_normal(mean, cov, len(Xin)))
    ...
    def update_motion_improved(self, mean, cov, poleparams):
        for i in range(self.count):
            ...
            x_dyn = self.motion(self.particles_pre[i], mean, cov) # modified
            self.particles_dyn[i] = x_dyn # modified
            x_new = x_out + np.random.multivariate_normal([0,0,0],Pout,1).T 
            self.particles[i] = util.xyp2ht(x_new.reshape(-1)) # modified 
            self.Poutt[i] = Pout
            self.xukf[i] = x_out
    ...
    def update_measurement_improved(self, poleparams, resample=True):
        ...
        for i in range(self.count):
            ...
            lik = np.prod(self.poledist.pdf(np.clip(d, 0.0, self.d_max)) + 0.1) # modified
            prior = scipy.stats.multivariate_normal(mean=[0,0,0], cov=self.Pukf[i,:,:]).pdf(util.ht2xyp(self.particles[i]) - util.ht2xyp(self.particles_dyn[i]))# modified
            proposal = scipy.stats.multivariate_normal(mean=[0,0,0], cov=self.Poutt[i]).pdf(util.ht2xyp(self.particles[i]) - self.xukf.reshape(-1)) # modified
    ...
    def ukf(self, Xin, Pin, mean, cov, poleparams):
        ...
        for i in range (nsp):
            ...
            x_pred = util.ht2xyp(x_pred).reshape(states,1) # modified
